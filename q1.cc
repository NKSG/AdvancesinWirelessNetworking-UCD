...
PhyTxTrace (std::string context, Ptr<const Packet> packet, WifiMode mode, WifiPreamble preamble, uint8_t txPower)
{
  if (g_verbose)
    {
	t = clock();
	if(i == (int) t/CLOCKS_PER_SEC){
		std::cout << mode << " " <<(float) t/CLOCKS_PER_SEC << std::endl;
		i++;
	}
      //std::cout << "PHYTX mode=" << mode << std::endl;//" " << *packet << std::endl;
    }
}
...
int main (int argc, char *argv[])
{
  ...
  Simulator::Schedule (Seconds (1.0), &AdvancePosition, stas.Get (0));
        ...
        if(proto[0] == 'u')
        {
                 ...
                 apps = client.Install (stas.Get (0));
                 apps.Start (Seconds (0.1));
                 apps.Stop (Seconds (350.0));
